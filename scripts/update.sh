#!/usr/bin/env bash
#
# Updates an existing health-service install to the latest GitHub release.
#
...
# first if health-service isn't installed yet.
#
...
# Canonical set of env vars health-service expects, with their defaults from
# .env.dist -- used only for whatever's missing from the existing unit.
ENV_KEYS=(MODE LOG_LEVEL MESSAGE_BUS_URL MESSAGE_BUS_EXCHANGE MESSAGE_BUS_ROUTING_KEY)
...
main() {
  ...
  if [ ! -f "$INSTALL_PATH" ] || [ ! -f "$UNIT_PATH" ]; then
    error "health-service doesn't look installed yet (missing ${INSTALL_PATH} or ${UNIT_PATH})"
  ...
  echo
  info "about to update health-service (${current_tag} -> ${tag}) with:"
  for key in "${ENV_KEYS[@]}"; do
    printf '    %-22s %s\n' "$key" "${final_env[$key]}"
  done
  echo
...
Environment=MESSAGE_BUS_ROUTING_KEY=${final_env[MESSAGE_BUS_ROUTING_KEY]}
